...
> **架构关联：** 本文档中的原则已整合到 `AI_UNIFIED_ARCHITECTURE.md` 统一架构文档中。
> 如需了解完整的架构规范、实施细节和代码示例，请参阅该文档。
...
在结构化自由架构下，AI 不再是系统的"大脑"，而是：

| 传统角色 | 结构化自由角色 |
|----------|----------------|
| 复杂推理引擎 | **意图路由器（Intent Router）** |
| 数据处理器 | **语义渲染器（Semantic Presenter）** |
| 逻辑决策者 | **工具调度器（Tool Dispatcher）** |
...
### 原则 20：事实单向性原则（Fact One-Way Principle）

> **事实只能从系统流向 AI，不能从 AI 回流到系统。**

AI 的任何输出，若被写回数据库、缓存或状态机，必须经过工具层的结构化确认与校验。

**禁止将 AI 自然语言输出，直接作为系统事实来源。**
...
| 风险场景 | 后果 |
|----------|------|
| AI 编造客户名称写入数据库 | 产生虚假客户记录 |
| AI 猜测金额写入结算表 | 财务数据错误 |
| AI 推断关系写入关联表 | 业务逻辑混乱 |

**正确做法：**

1. AI 只能**建议**数据，不能直接写入
2. 写入操作必须通过**工具**执行
3. 工具必须对数据进行**结构化校验**
4. 关键操作需要**用户确认**
...
| 20 | **事实单向性** | **事实从系统→AI，不能回流** |
| 21 | 幻觉根源 | AI 处理了不该处理的事 |
| 22 | 数据不经 AI 加工 | 从工具直接到输出 |
| 23 | 返回值可直接使用 | 无需二次处理 |
| 24 | AI 不能操作 = 失败设计 | 终极判断标准 |
| 25 | 幻觉 = 系统失败 | 不责怪 AI，改进系统 |
